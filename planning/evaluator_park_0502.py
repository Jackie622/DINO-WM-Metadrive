...
class PlanEvaluator:  # evaluator for planning
    ...
    def _get_traj_last_n(self, traj_data, length, n):
        """Extract the last n frames for each trajectory.
        traj_data: (B, T, ...) tensor or numpy array
        length: (B,) — if inf, use full trajectory length T
        Returns: (B, n, ...) same type as traj_data
        """
        B, T = traj_data.shape[0], traj_data.shape[1]
        last = np.where(np.isinf(length), T, length.astype(int))  # last valid index (exclusive)
        start = np.maximum(0, last - n).astype(int)

        is_np = not isinstance(traj_data, torch.Tensor)
        result = []
        for b in range(B):
            frames = traj_data[b, start[b]:last[b]]
            # pad front if less than n frames (e.g. at the very start of rollout)
            if frames.shape[0] < n:
                pad_slices = n - frames.shape[0]
                if is_np:
                    pad = np.repeat(frames[0:1], pad_slices, axis=0)
                    frames = np.concatenate([pad, frames], axis=0)
                else:
                    pad = frames[0:1].repeat(pad_slices, dim=0)
                    frames = torch.cat([pad, frames], dim=0)
            result.append(frames)

        return np.stack(result) if is_np else torch.stack(result)

    def _get_trajdict_last_n(self, dct, length, n=3):
        """Extract the last n frames spaced frameskip apart for each trajectory (dict version).

        Unlike _get_traj_last_n which takes consecutive frames, this method takes frames
        at positions [last-1, last-frameskip-1, last-2*frameskip-1, ...]
        so the obs frames match the frameskip spacing the WM was trained on.
        """
        new_dct = {}
        for key, value in dct.items():
            new_dct[key] = self._get_traj_last_n_spaced(value, length, n)
        return new_dct

    def _get_traj_last_n_spaced(self, traj_data, length, n):
        """Extract last n frames with frameskip spacing.
        ...
        """
        B, T = traj_data.shape[0], traj_data.shape[1]
        last_excl = np.where(np.isinf(length), T, length.astype(int))
        # Take 3 frames at positions spaced by frameskip:
        # [last - 2*step - 1, last - step - 1, last - 1]
        # matching the frameskip-spaced training data (the WM expects 3 frames
        # with significant visual change between them).
        step = self.frameskip
        indices = np.array([
            np.maximum(0, last_excl - (n - 1 - i) * step - 1)
            for i in range(n)
        ]).T  # (B, n)
