...
class PlanWorkspace:
    ...
    def perform_planning(self):
        ...
        if is_hierarchical:
            ...
            # 🟢 保存原始初始条件（用于 Phase 2 的 env 回放）
            original_obs_0 = self.obs_0
            original_state_0 = self.state_0
            ...
            self.planner.reset()
            self.planner.logging_prefix = "mpc_phase2"

            # 🚀 Phase 2: config-driven sub-planner override
            # 从 YAML 配置读取 Phase 2 planner 参数（不再硬编码）
            phase2_cfg = self.cfg_dict.get("phase2")
            if phase2_cfg is None:
                # No Phase 2 config: use the same sub-planner as Phase 1
                logging_prefix = "phase2_same"
                self.planner.n_taken_actions = 3
            else:
                cfg_sub = phase2_cfg["sub_planner"]
                cfg_sub["_target_"] = cfg_sub["target"]
                old_sub = self.planner.sub_planner
                self.planner.sub_planner = hydra.utils.instantiate(
                    cfg_sub,
                    wm=old_sub.wm,
                    action_dim=old_sub.action_dim,
                    objective_fn=old_sub.objective_fn,
                    preprocessor=old_sub.preprocessor,
                    evaluator=old_sub.evaluator,
                    wandb_run=old_sub.wandb_run,
                    logging_prefix=phase2_cfg.get("logging_prefix", "phase2"),
                    log_filename=None,
                )
                self.planner.n_taken_actions = phase2_cfg["n_taken_actions"]

            # 🔥🔥🔥 核心修复：不再传送！
            #
            # 旧方案：通过 teleport 让 env 跳到 Phase 1 终点 → set_velocity(0) 破坏连续性，
            #        同时 WM 的 obs_0 是带原始速度的 3 帧 → 编码-解码崩塌。
            #
            # 新方案：与 Phase 1 内部迭代间对齐完全一致：
            #   1. base_history = actions1（Phase 1 全部动作）
            #   2. evaluator init_cond = 原始起点（original_obs_0, original_state_0）
            #   3. MPC 内部 eval 时，env 从原始起点回放 Phase 1 + Phase 2 所有动作，
            #      然后把 Phase 1 切片切掉 → env 和 WM 自然对齐
            #
            # CEM 的 WM 起点仍用 phase1_final_obs（3 帧真实 env 画面），
            # 和 Phase 1 每次迭代间 CEM 的行为完全一致。
            self.planner.base_history = actions1.detach()
            self.evaluator.history_actions = None  # MPC plan 内部会通过 base_history 设置
            self.evaluator.assign_init_cond(obs_0=original_obs_0, state_0=original_state_0)
            self.evaluator.obs_g = self.obs_g
            self.evaluator.state_g = self.state_g

            # 🔥🔥🔥 核心修复：将 Phase 1 的最后 2 帧归一化动作传给 ClampedWM
            #
            # obs_0 (phase1_final_obs) 包含 3 帧，位置在 [5*(T-2), 5*(T-1), 5*T]
            # 第 i 帧需要配对"从该帧出发"的宏动作作为 encode(frame[i], act[i])：
            #   frame[0] (5*(T-2))  → 从它出去的动作为 actions[-2]
            #   frame[1] (5*(T-1))  → 从它出去的动作为 actions[-1]
            #   frame[2] (5*T)      → 从它出去的动作为未来的 action[0]（未知）
            #   → 第三帧用 actions[-1] 复制填充作为最佳猜测
            num_hist = 3
            if actions1.shape[1] >= num_hist:
                wm = self.planner.sub_planner.wm
                if hasattr(wm, "init_actions"):
                    init_a = actions1[:, -2:, :].detach().cpu()  # (B, 2, D)
                    init_a = torch.cat([init_a, init_a[:, -1:, :]], dim=1)  # (B, 3, D)
        ...
        # 🔥 清除 init_actions：final_eval 从原始起点开始，不需要历史动作
        if hasattr(self.planner.sub_planner.wm, "init_actions"):
            self.planner.sub_planner.wm.init_actions = None

        prefix = getattr(self.planner, "logging_prefix", "final")
        logs, successes, _, _ = self.evaluator.eval_actions(
            actions.detach(), action_len, save_video=True, filename=f"{prefix}_final"
        )
